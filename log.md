...
> 对于select组件，目前觉的难的就是他是多个组件的一个集合体，真正实现起来只是复杂而已



> 今天我觉的比较复杂的是filter功能的实现：不是很难，就是很琐碎

1. 首先，分两步，接受外面的过滤函数还是使用自身的过滤函数，这样就是用函数封装进行判断
2. 需要对input组件绑定事件，当输入的时候，就是过滤，当然，每次过滤都是从props的原始option中进行过滤
3. 怎么样去控制过滤？
   1. 当展示下拉框的时候，如果是启动过滤，那么要做三件事情：
      - 清空当前的输入，直接清空就行
      - 把上一次的输入作为placehodelder，这里使用`filteredPlaceholder`单独的响应式变量计算，如果展示下拉框且开启过滤，如果不是第一次，就把上一次的输入作为placehodelder
      - 展示所有的内容，即重新过滤一次
   2. 当隐藏下拉框的时候，需要重新设计
      - 如果当前有上一次的输入，那么就直接用上一次的，而不是直接使用空字符串
      - 并且，此时当点击之后，需要使用readonly，即开启过滤，且关闭展示下拉框的时候。如果没有点击，那么会根据上一条直接覆盖掉当前的输入，而使用上一次的合法输入

> **支持远程搜索**

这是实际上就是在上面filter的基础上操作的，当打开的时候，此时filterOption是发送网络请求返回的，注意加上**防抖**就行



> 键盘操作

这里主要是注意使用e.key进行对按下的字符进行识别，当键盘按下的时候，会触发事件，传入e，e的一个类型是最新的`KeyboardEvent`，可以通过`额e.code/e.key`对触发的按键进行识别
...
实际上键盘操作也不难，只是之前没做过，且逻辑很繁琐，需要考虑的事情很多，这里以高亮下标重置为例子，在关闭之后，需要进行高亮下标的重置，不然下一次打开还是之前的高亮效果，并且，对于filter的时候，由于不断的筛选，此时item的数量也在变化，所以也需要在fliter之后进行高亮下标的重置

# 发布打包

具体步骤：

1. `npm whoami`先判断是不是登录状态，不是的话，使用`npm login`进行登录，登录完成之后推送

2. 发布之前需要修改包的版本，修改`package.json文件中`在`packages`中的version
